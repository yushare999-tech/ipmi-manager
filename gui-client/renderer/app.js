...
function renderDevices() {
    ...
    <div class="device-card" id="card-${d.id}">
      <div class="device-card-top">
        <div>
          ...
          <div class="device-ip">
            ${d.ipmi_ip}
            <span class="ping-badge ping-pending" id="ping-${d.id}" data-ip="${d.ipmi_ip}">Checking...</span>
          </div>
        </div>
  ...
  setTimeout(checkAllPings, 50);
}

// 핑 체크 실행 함수 (동시 실행 수를 3개로 제한하여 백그라운드 리소스 과점 방지)
async function checkAllPings() {
  const badges = document.querySelectorAll('.ping-badge');
  ...
  for (let i = 0; i < badgeArray.length; i += limit) {
    ...
    await Promise.all(chunk.map(async (badge) => {
      ...
      try {
        const res = await window.ipmiAPI.pingDevice(ip);
...
// JNLP: 실행 전 핑 체크 선행 후 온라인인 경우에만 기존 로직 진행
async function connectJnlp(id) {
  ...
  try {
    const pingRes = await window.ipmiAPI.pingDevice(d.ipmi_ip);
    if (pingRes.success && pingRes.online) {
      if (badge) {
        badge.className = 'ping-badge ping-online';
        badge.textContent = 'Online';
      }
      ...
      alert(`[오프라인] 장비(${d.ipmi_ip})와 통신할 수 없습니다. JNLP 실행을 취소합니다.`);
    }
  } catch (e) {
    if (badge) {
      badge.className = 'ping-badge ping-offline';
...
$('btn-import-api').addEventListener('click', async () => {
  ...
  try {
    ...
    // 1. 응답 데이터에서 장비 목록 추출 (배열 자체이거나 .devices인 경우 둘 다 대응)
    let deviceList = [];
    ...
    if (deviceList.length > 0) {
      // 'type'이 'ipmi'인 장비만 필터링하여 가져옴
      const ipmiDevices = deviceList.filter(d => (d.type || '').toLowerCase() === 'ipmi');

      // 2. API 규격을 우리 프로그램 데이터 규격으로 변환 및 매핑
      const imported = ipmiDevices.map(d => {
        const serial = d.js_serial || d.id || uid();
        const vendorName = (d.vendor || 'generic').toLowerCase();
        
        return {
          id:       serial,
          name:     d.js_serial ? `[${d.js_serial}] ${d.vendor} ${d.model || ''}`.trim() : (d.name || d.ip || d.ipmi_ip),
          ipmi_ip:  d.ip || d.ipmi_ip || '',
          vendor:   vendorName === 'hp' ? 'hp' : (vendorName === 'hpe' ? 'hp' : vendorName),
          model:    d.model || '',
          username: d.username || '',
          password: d.password || '',
          note:     d.network_memo || d.note || '',
          https:    d.https !== false,
        };
      });
      
      // 기존 장비 매핑
      const existingMap = new Map(state.devices.map(d => [d.id, d]));
      let newCount = 0;
      let updateCount = 0;
      ...
      let overwrite = false;
      if (updateCount > 0) {
        overwrite = confirm(`이미 등록된 장비 중 ${updateCount}개의 정보가 API 서버에서 수정된 것으로 확인되었습니다.\n\n기존 장비의 설정을 최신 API 정보로 덮어쓰시겠습니까?\n\n(취소 시, 기존 설정은 유지하고 신규 장비 ${newCount}개만 추가합니다.)`);
      } else if (newCount === 0) {
        alert('ℹ️ 이미 모든 장비가 최신 상태입니다. 추가하거나 변경할 장비가 없습니다.');
        return;
      }

      if (overwrite) {
        // 덮어쓰기 활성화: API 데이터로 업데이트 및 신규 추가
        imported.forEach(d => {
          existingMap.set(d.id, d);
        });
        state.devices = Array.from(existingMap.values());
      } else {
        // 덮어쓰기 비활성화: 신규 장비만 필터링하여 추가
        const newDevices = imported.filter(d => !existingMap.has(d.id));
      ...
      if (overwrite) {
        alert(`✅ 신규 장비 ${newCount}개 추가 및 기존 장비 ${updateCount}개의 정보가 업데이트(덮어쓰기)되었습니다.`);
      } else {
        alert(`✅ 신규 장비 ${newCount}개를 추가했습니다. (기존 장비 정보 유지)`);
      }
    } else {
      alert('⚠️ API 응답 형식이 올바르지 않거나 장비 목록이 비어 있습니다.');
    }
  } catch (e) {
...
async function init() {
  // 앱 버전 표시
  if (window.ipmiAPI && window.ipmiAPI.getAppVersion) {
    try {
      const version = await window.ipmiAPI.getAppVersion();
      const versionEl = $('app-version');
      if (versionEl) versionEl.textContent = `v${version}`;
    } catch (e) {
      console.warn('[Init] 버전 로드 실패:', e);
    }
  }
  ...
  // 핑 재체크 버튼 바인딩
  const refreshBtn = $('btn-refresh-ping');
  if (refreshBtn) {
    refreshBtn.addEventListener('click', checkAllPings);
  }
