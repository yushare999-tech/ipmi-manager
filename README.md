...
├── go-daemon/           # Go 기반 로컬 서비스 데몬 (자바/웹 뷰어 기동 백그라운드 대행)
│   ├── viewer/          # [NEW] 초경량 Go WebView2 크로미움 웹 뷰어 소스 폴더
│   │   └── main.go      # 웹 뷰어 엔트리 포인트 (인증서 에러 우회, 로그인 폼 자동 주입)
│   ├── build.ps1        # [NEW] 프로세스 락 자동 해제 및 자동 버전업 통합 빌드 스크립트
│   ├── version.go       # [NEW] 버전 카운팅을 관리하는 버전 정보 소스 파일
...
### v1.5.4 (2026-07-01)
- **초경량 Go 크로미움 웹 뷰어(`ipmi-viewer.exe`) 도입**: 기존 Electron(Node.js)의 100MB 이상 무거운 용량 대신 Windows WebView2 엔진을 타는 5MB 수준의 초경량 단독 뷰어 기동 체계로 변경.
- **인증서 에러 강제 우회 및 자동 로그인**: 사설 SSL/TLS 인증서 무시 옵션을 크로미움에 자동 주입하고, 벤더별(Dell, Supermicro, HP) 웹 콘솔 폼 요소를 감지하여 ID/PW를 자동 주입 후 로그인 시키는 기법 완비.
- **프로세스 락 해제 및 버전업 통합 빌드 자동화 (`build.ps1`)**: 빌드 가동 시 기존 구동 중인 데몬을 자동 종료시켜 파일 잠금을 해제하며, 버전 정보를 자동으로 1 카운트업(v1.5.4) 한 뒤, 빌드 성공 시에만 최종 데몬을 안전하게 재구동하는 빌드 자동화 구축.
